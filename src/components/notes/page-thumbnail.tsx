...
export function PageThumbnail({ page }: { page: PageRecord }) {
  ...
  useEffect(() => {
    ...
    async function draw() {
      ...
      if (notebook?.pdfAssetId && page.pdfPage) {
        try {
          const document = await loadStoredPdfDocument(notebook.pdfAssetId);
  ...
  return <canvas ref={canvasRef} className="block h-auto w-full rounded-sm bg-paper" aria-hidden />;
}
...
async function drawImage(
  ...
  const image = await loadAssetImage(object.assetId).catch(() => null);
  if (!image) return;
  context.save();
