# Hướng dẫn Phát hành & Tự động Cập nhật Mtrii Notes qua GitHub

Tính năng tự động kiểm tra và cập nhật của **Mtrii Notes** hoạt động dựa trên **GitHub Releases API**. Mỗi khi ứng dụng khởi động (hoặc khi bạn bấm "Kiểm tra bản cập nhật mới" trong Cài đặt), ứng dụng sẽ tự động kiểm tra xem có phiên bản mới hơn trên GitHub không.

---

## 1. Cấu hình GitHub Repository trong ứng dụng

1. Mở **Mtrii Notes** -> Vào mục **Cài đặt** (Settings).
2. Cuộn xuống phần **Giới thiệu và cập nhật**.
3. Tại ô **GitHub Repository**, nhập địa chỉ kho mã nguồn của bạn:
   - Ví dụ: `username/mtrii-notes` hoặc `https://github.com/username/mtrii-notes`.
...
3. Sau khi đóng gói hoàn tất, file cài đặt sẽ nằm tại:
   `src-tauri/target/release/bundle/nsis/Mtrii Notes_x.x.x_x64-setup.exe`

---

## 3. Cách tạo Release trên GitHub để người dùng nhận cập nhật

1. Truy cập vào Repository của bạn trên GitHub (`https://github.com/username/mtrii-notes`).
...
3. Điền các thông tin:
   ...
   - **Release title**: Tên bản phát hành (ví dụ: `Mtrii Notes v0.2.1 - Cập nhật giao diện và tính năng mới`).
...
4. **Attach binaries**: Kéo thả file `Mtrii Notes_x.x.x_x64-setup.exe` vào ô đính kèm tài nguyên (Assets).
5. Nhấn **Publish release**.

---

## 4. Trải nghiệm cập nhật trên Mtrii Notes
