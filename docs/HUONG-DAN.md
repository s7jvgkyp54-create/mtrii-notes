# Notes — hướng dẫn chạy, bảo trì và chuyển máy

Ứng dụng sổ tay số **Notes** (`com.notes.app`). Đây **không** phải Goodnotes.
...
3. Máy mới: mở Notes tương thích → Nhập bản sao lưu → xem số sổ/trang → xác nhận.
...
- `Notes_0.2.0_x64-setup.exe`: bộ cài NSIS cho tài khoản Windows hiện tại, có Start Menu và trình gỡ cài đặt.
- `Notes_0.2.0_x64-portable.exe`: bản chạy trực tiếp để kiểm tra, không thay thế bộ cài khi dùng lâu dài.
...
- SQLite: `%LOCALAPPDATA%\com.notes.app\notes.sqlite3`.
- PDF/ảnh/âm thanh: `%LOCALAPPDATA%\com.notes.app\assets\`.
- Backup tự động: `%LOCALAPPDATA%\com.notes.app\backups\`.
...
Định danh `com.notes.app` và đường dẫn kho phải được giữ ổn định giữa các bản. Trước migration DB: tạo backup khôi phục được. Migration lỗi phải dừng, không tạo DB rỗng đè dữ liệu.
...
- Không tuyên bố đạt “99% Goodnotes”; Notes dùng thương hiệu, định dạng backup và cách triển khai riêng.
